...
public abstract class TreeList<TreeType extends Tree> extends Tree {
    ...
    // Rajout de cette méthode pour void decompile() dans ListExpr()
    public TreeType getLast() {
        if (list.isEmpty()) {
            return null;
        }
    ...
    // Définition de la méthode addAll
    public void addAll(TreeList<? extends TreeType> elements) {
        Validate.notNull(elements);
        for (TreeType element : elements.getList()) {
            list.add(element);
        }
    }
