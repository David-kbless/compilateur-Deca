...
@init   {
    ...
    : i=ident {
            assert($i.tree != null);
            AbstractIdentifier identTree = $i.tree;
            org.antlr.v4.runtime.Token identToken = $i.start;
        }
      (EQUALS e=expr {
            assert($e.tree != null);
            initExpr = $e.tree;
...
@init {
    ...
    : if1=IF OPARENT condition=expr CPARENT OBRACE li_if=list_inst CBRACE {
      ...
      (ELSE elsif=IF OPARENT elsif_cond=expr CPARENT OBRACE elsif_li=list_inst CBRACE {
            ...
            IfThenElse elsifNode = new IfThenElse($elsif_cond.tree, $elsif_li.tree, new ListInst());
            setLocation(elsifNode, $elsif);
...
literal returns[AbstractExpr tree]
    : INT {
        ...
        long val = Long.parseLong($INT.text);
        $tree = new IntLiteral(val);
      ...
      | decl_field_set
        {
            ...
            $fields.addAll($decl_field_set.tree);
        }
      )*
    ;
    ...
    : v=visibility t=type list_decl_field
      SEMI { 
        for (AbstractDeclField field : $list_decl_field.tree.getList()) {
            ((DeclField)field).setVisibility($v.tree);
            ((DeclField)field).setType($t.tree);
            assert(field != null);
            $tree.add(field);
        }
        }
...
decl_field returns [DeclField tree]
    : i=ident init=initialization {
            assert($i.tree != null);
            assert($init.tree != null);
            $tree = new DeclField($i.tree, $init.tree);
            setLocation($tree, $i.start);
        }
    ;

initialization returns[AbstractInitialization tree]
    : EQUALS e=expr {
            assert($e.tree != null);
            $tree = new Initialization($e.tree);
            setLocation($tree, $EQUALS);
        }
    | /* epsilon */ {
...
@init {
    ...
    : v= visibility type ident OPARENT params=list_params CPARENT (block {
        $tree = new DeclMethod($v.tree, $type.tree, $ident.tree, $params.tree, $block.decls, $block.insts);
        setLocation($tree, $ident.start);
        }
      | ASM OPARENT code=multi_line_string CPARENT SEMI {
        assert($code.text != null);
        $tree = new DeclMethod($v.tree, $type.tree, $ident.tree, $params.tree, new ListDeclVar(), new ListInst(), $code.text);
        setLocation($tree, $ident.start);
        }
      ) {
        }
